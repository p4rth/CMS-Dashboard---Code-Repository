...
public class JobViewActivity extends Activity {
	...
	ArrayList<JobsModel> tasksArray = null;
	JobsAdapter tasksadapter;
	JobsList list;
	JobsSync loadTaskThread;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tasksArray = new ArrayList<JobsModel>();
		tasksadapter = new JobsAdapter(JobViewActivity.this,R.layout.job_list_item,tasksArray);
	...
	private class JobsSync extends AsyncTask<String, Integer, JobsList>{
		...
		protected JobsList doInBackground(String... urls) {
			...
			try{
				...
				Gson gson = new Gson();
				list = gson.fromJson(jsonStr, JobsList.class);		//should be jobs???	
			}
		...
		protected void onPostExecute(JobsList taskslist)
		{
			if(taskslist.jobs.size()==0)
			{
			...
			for(JobsModel tm: taskslist.jobs)
			{
				tasksArray.add(tm);
			}
